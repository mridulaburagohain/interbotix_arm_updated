...
This Repository is forked from the Interbotix toolboxes repository and modified for our use for easy manipulation
...
│   └── ...
```
The addition and modification of new features to the moveit controlled  GUI are done by modifying the *interbotix_moveit_interface* and *interbotix_moveit_interface_msgs* modules using the above-mentioned files. 
- **interbotix_moveit_interface** - This module is responsible to initialize to the moveGroupInterface, create services like moveit_plan and clear_markers and uses the GUI interpretations to control the arm.
     - Previously, the InterbotixMoveitInterface class controlled only the arm without the gripper control and returned a boolean value on any command execution call from the GUI.
     - While the modified code has added a joint_states subscriber to check and update joint states before trajectory execution if an error threshold is surpassed, added collision objects using moveitPlanningScene to include environmental objects in its operation region, added the saved/obtained path as a msg response to the GUI calls to be recorded for later use, added planning group change abilities during the control operation to enable gripper control with arm, added functions to retrace and repeat trajectories and gripper actions on GUI call.
     - The collision object created can serve as a base for future additions into the environment
...
Please find more details in the ```README``` file of the respective module
